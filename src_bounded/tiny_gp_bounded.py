    ...
    for gen in range(1, GENERATIONS + 1):  
        ...
        while len(new_pop) < POP_SIZE:
            ...
            if prob < XO_RATE:
                ...
                if IODC(max_IODC, z, parent, train_dataset) < MAX_COMPLEXITY:
                    new_pop.append(parent)

                if len(new_pop) < POP_SIZE and IODC(max_IODC, z, parent2, train_dataset) < MAX_COMPLEXITY:
        ...
        if not all(comp < MAX_COMPLEXITY for comp in iodc_distribution[-1]):
            print('Last gen complexities:', iodc_distribution[-1])
            raise Exception('Individual exceeding bound...')
