...
def target_based_complexity(ind, dataset, target):
    complexity = 0
    ...
    for j in used_feats:
        ...
        # Calculate the mean of the slopes difference across the i=1..n-2 sum -> TODO: This is dividing by the totoal number os observations - 2. Is this correct?
        # If we divide by the length of q_j instead, less complex features (more constant ones) will have a smaller denominator, which might increase their contribution to the complexity.
...
def slope_based_complexity(ind, dataset, target):
