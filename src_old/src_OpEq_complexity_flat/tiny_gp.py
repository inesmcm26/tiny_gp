...
from configs_OpEq_flat import *
from gptree import GPTree
from complexity_measures_flat import init_IODC, IODC
from opEq_flat import init_target_hist, init_hist, update_target_hist, reset_pop_hist, check_bin_capacity, update_hist, get_population_len_histogram, get_bin
...
def init_population(terminals, max_IODC, z, train_dataset):
    ...
    for max_depth in range(MIN_DEPTH, MAX_INITIAL_DEPTH + 1):
        ...
        for _ in range(inds_per_depth):
            ...
            # Only create individuals with lower complexity than target
            if IODC(max_IODC, z, ind, train_dataset) < 1:
                pop.append(ind)
...
def evolve(train_dataset, test_dataset, train_target, test_target, terminals):
    ...
    pop_hist_fitnesses = init_hist(population, train_fitnesses, max_IODC, z, train_dataset)
    ...
    print('INITIAL BEST inv IODC:', iodc[0])
    # Save distributions
