...
from configs_nested import GENERATIONS
...
def run_stdGP(ds_name):
    ...
    SAVE_PATH = f'/home/ines/Documents/tese/tiny_gp/results_nested_sampled/{ds_name}/'
    ...
    for run_nr in tqdm(range(21, 31)): # TODO: CHANGE HERE!
        ...
        train_dataset, test_dataset, sampled_dataset, train_target, test_target = read_dataset(ds_name, run_nr, sampling_mode = 'subsampled')
        ...
        best_train_fit_list, best_test_fit_list, best_ind_list, best_of_run_gen, \
            mean_train_fit_list, mean_test_fit_list, \
                 slope_list, \
                        slope_sampled_list, \
                            slope_test_list, \
                            features_contribution_list, features_contribution_sampled_list, features_contribution_test_list, \
            ...
            evolve(train_dataset, test_dataset, sampled_dataset, train_target, test_target, terminals)
        ...
        features_contribution_sampled = pd.DataFrame([features_contribution_sampled_list], columns = [i for i in range(0, GENERATIONS + 1)])
        ...
        slope_sampled = pd.DataFrame([slope_sampled_list], columns = [i for i in range(0, GENERATIONS + 1)])
        # mean_slope_sampled = pd.DataFrame([mean_slope_sampled_list], columns = [i for i in range(0, GENERATIONS + 1)])
        # slope_sampled_distribution = pd.DataFrame([slope_sampled_distribution_list], columns = [i for i in range(0, GENERATIONS + 1)])
        ...
        slope_sampled.to_csv(SAVE_PATH + f'slope_sampled_complexity_run{run_nr}.csv')
        # mean_slope_sampled.to_csv(SAVE_PATH + f'mean_slope_sampled_complexity_run{run_nr}.csv')
        # slope_sampled_distribution.to_csv(SAVE_PATH + f'slope_sampled_distribution_run{run_nr}.csv')
        ...
        features_contribution_sampled.to_csv(SAVE_PATH + f'features_contribution_sampled_run{run_nr}.csv')
