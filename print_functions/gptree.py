...
class GPTree:
    ...
    def math_expr_to_tree_expr(self, math_expr):
        """
        Transform a mathematical expression to a tree expression using custom functions.
        Args:
            math_expr (str): Mathematical expression, e.g., 'x1+x2'.
        Returns:
            str: Tree expression using custom functions, e.g., 'add(x1, x2)'.
        """
        # Replace operator symbols with function names
        for op, func_name in MAPPING.items():
            math_expr = math_expr.replace(op, func_name)

        # Evaluate the modified expression using eval
        return eval(math_expr, FUNCTIONS.__dict__)
    ...
    def create_lambda_function_from_string(self, string_expr):
